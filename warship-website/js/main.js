document.addEventListener("DOMContentLoaded", function () {
  ...
  gunButton.addEventListener("click", function () {
    ...
    modelViewer.cameraOrbit = "300deg 75deg 10deg"; // Adjust the rotation accordingly

    // Set the camera target to the gun's position
    modelViewer.cameraTarget = { x: 40, y: 0, z: 10 }; // Adjust the position accordingly
    ...
    modelViewer.style.transform = "scale(2.5)"; // Adjust the scale factor for desired zoom level

    // Move the camera to the new position
    modelViewer.cameraChangeView(); // This will trigger the camera to move immediately
  });
});
